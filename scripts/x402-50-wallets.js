/**
 * x402 50 Wallets Stress Test
 * 
 * 1. Generates 50 wallets
 * 2. Disperses SOL + USDC from master wallet
 * 3. Runs x402 transactions every 2 minutes
 * 
 * Usage:
 *   node scripts/x402-50-wallets.js --generate    Generate 50 wallets
 *   node scripts/x402-50-wallets.js --disperse    Disperse funds from master
 *   node scripts/x402-50-wallets.js --run         Run the stress test
 *   node scripts/x402-50-wallets.js --status      Check all balances
 */
...
// Configuration
const WALLETS_FILE = path.join(__dirname, 'x402-50-keypairs.json');
const WALLET_COUNT = 50;
const INTERVAL_MS = 2 * 60 * 1000; // 2 minutes

// Master wallet (funded by user)
const MASTER_PRIVATE_KEY = '36UyuUTUBtMWMFmq6xu5sfpNC9VpgLnFFcZjZB1D39KdVtSLnBcvuWyumiuGamzU8rEQtEroc9qagS8BxCmL4UiD';
...
const SOL_PER_WALLET = 0.003; // 0.003 SOL for rent + fees
const USDC_PER_WALLET = 0.02; // 0.02 USDC (enough for 2 txs)
...
/**
 * Disperse funds from master wallet to all wallets
 */
...
async function runStressTest() {
  ...
  console.log('\n🚀 Starting x402 Stress Test (50 wallets)');
...
Master Wallet: 9m3yXKpqNiiFQd6HWp6QYqcgMEq1mD9rZT6cUqxcqtHo

Commands:
  --generate   Generate 50 wallets
...
Steps:
  1. Send SOL (~0.2) + USDC (~1.5) to master wallet
  2. Run: node scripts/x402-50-wallets.js --generate
  3. Run: node scripts/x402-50-wallets.js --disperse
  4. Run: node scripts/x402-50-wallets.js --status
  5. Run: node scripts/x402-50-wallets.js --run
`);
  }
}
